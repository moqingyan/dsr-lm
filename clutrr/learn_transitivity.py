...
class CLUTRRModel(nn.Module):
  def __init__(
    ...
    # Transitivity probs
    self.transitivity_probs = torch.tensor(torch.rand(len(all_possible_transitives))/2, requires_grad=True)
  ...
  def forward(self, x, phase='train'):
    ...
    transitive_relations = [[(prob, relation) for prob, relation in zip(self.transitivity_probs, all_possible_transitives)] for _ in range(len(relations))]
...
if __name__ == "__main__":
  ...
  parser.add_argument("--model-name", type=str, default="learn_transitivity_init_0.5")
